...
class DiffusionVisionTower(BaseVisionTower):
    ...
    def load_model(self, device_map=None):
        ...
        # Added these for ProcessWrapper
    ...
    def _forward(self, images, **kwargs):
        with torch.set_grad_enabled(
            ...
            time_step = self._config.get("time_step", 25)

            unet_output = self.extract_raw_features(
                images=images.to(device=self.device, dtype=self.dtype),
                prompt_embeds=None,
                time_step=time_step
            )

            bottleneck_features = unet_output["bottleneck"][0]
            image_features = bottleneck_features.mean(dim=[2, 3])
...
class DiTTower(BaseVisionTower):
    ...
    def load_model(self, device_map=None):
        ...
        # Have to add there to not break ProcessWrapper
        self.image_processor.image_mean = [0.5, 0.5, 0.5]
        self.image_processor.image_std = [0.5, 0.5, 0.5]
    ...
    def _forward(self, images, **kwargs):
        with torch.set_grad_enabled(
            ...
            all_layer_features = self.extract_raw_features(
                images.to(device=self.device, dtype=self.dtype),
                prompt_embeds=None,
                time_step=time_step
            )

            layer_idx = self._config.get("mm_vision_select_layer", -1)
            if layer_idx == -1:
                layer_idx = all_layer_features.shape[1] - 1
  
            selected_layer_features = all_layer_features[:, layer_idx, :, :]
            image_features = torch.mean(selected_layer_features, dim=1)
