...
class ProcessorWrapper:
    ...
    def preprocess(self, image, return_tensors='pt', images=None):
        if images is not None:
            image = images[0] if isinstance(images, (list, tuple)) else images
        output = {"pixel_values": [self._transforms(image)]}
        if return_tensors == "pt" and len(output["pixel_values"]) == 1:
            output["pixel_values"] = torch.stack(output["pixel_values"])
        return output
    ...
    def __call__(self, images=None, return_tensors="pt", **kwargs):
        ...
        if images is None:
            raise ValueError("ProcessorWrapper requires images=...")
        if not isinstance(images, (list, tuple)):
            images = [images]
        pixel_values = [self._transforms(img) for img in images]
...
class BaseVisionTower(nn.Module):
    def __init__(self, vision_tower_name, args, delay_load=False):
        ...
        if isinstance(args, dict):
            ...
            self.unfreeze_mm_vision_tower = args.get("unfreeze_mm_vision_tower", False)
        else:
            self.select_layer = args.mm_vision_select_layer
    ...
    def forward(self, images, **kwargs):
        if type(images) is list:
            image_features = [
                self._forward(image.unsqueeze(0), **kwargs)
                for image in images
            ]
        else:
            image_features = self._forward(images, **kwargs)
